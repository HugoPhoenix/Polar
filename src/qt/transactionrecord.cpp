...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZPAModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zpaTracker->HasMintTx(hash))) {
            //zPA stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZPA;
...
bool IsZPAType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZPA:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZPA || type == TransactionRecord::MNReward) {
