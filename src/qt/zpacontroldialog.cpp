...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zpaTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zpaTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zPA.";
            else if (!mint.isSeedCorrect)
                strReason = "The zPA seed used to mint this zPA is not the same as currently hold in the wallet";
            else
