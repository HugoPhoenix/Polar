...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
    if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zPA stake block is less than allowed start height", __func__);

    if (CZPivStake* zPA = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zPA->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zPA stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zPA stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zPA->GetValue()));
        if (nChecksum200 != zPA->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zPA->GetChecksum(), nChecksum200);
    } else {
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight)
{
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s: staked zPA fails context checks", __func__);
    } else {
        ...
        CPivStake* paInput = new CPivStake();
        paInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(paInput);
    }
    ...
    if (!txin.IsZerocoinSpend() && nPreviousBlockHeight >= Params().Zerocoin_Block_Public_Spend_Enabled() - 1) { //Equivalent for zPA is checked above in ContextualCheckZerocoinStake()
