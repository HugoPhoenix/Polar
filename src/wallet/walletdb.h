...
public:
    ...
    bool WriteZPASeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZPASeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZPASeed_deprecated(uint256& seed);
    bool EraseZPASeed();
    bool EraseZPASeed_deprecated();

    bool WriteZPACount(const uint32_t& nCount);
    bool ReadZPACount(uint32_t& nCount);
