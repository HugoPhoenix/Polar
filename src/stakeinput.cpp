...
// The zPA block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zPA is a hash of the serial
...
{
    //Create an output returning the zPA that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZPAOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zPA output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zPA", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZPAOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zPA output", __func__);
...
{
    CzPATracker* zpaTracker = pwallet->zpaTracker.get();
    CMintMeta meta;
    if (!zpaTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zpaTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a PA stake is the outpoint
