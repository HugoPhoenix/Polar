...
CzPATracker::CzPATracker(std::string strWalletFile)
{
...
CzPATracker::~CzPATracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzPATracker::Init()
{
...
bool CzPATracker::Archive(CMintMeta& meta)
{
...
bool CzPATracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzPATracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzPATracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzPATracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzPATracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzPATracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzPATracker::GetSerialHashes()
{
...
CAmount CzPATracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzPATracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzPATracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzPATracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzPATracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzPATracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzPATracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzPATracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzPATracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzPATracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzPATracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzPAWallet* zPAWallet)
{
    bool iszPAWalletInitialized = (NULL != zPAWallet);
    CMintMeta meta;
    ...
    if (! iszPAWalletInitialized)
        zPAWallet = new CzPAWallet(strWalletFile);
    meta.isSeedCorrect = zPAWallet->CheckSeed(dMint);
    if (! iszPAWalletInitialized)
        delete zPAWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzPATracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPATracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzPATracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzPATracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzPATracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzPATracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzPAWallet* zPAWallet = new CzPAWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zPAWallet);
        }
        delete zPAWallet;
        LogPrint("zero", "%s: added %d dzpa from DB\n", __func__, listDeterministicDB.size());
    }
