...
CzPAWallet::CzPAWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zpa seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPASeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPASeed_deprecated()) {
                    LogPrintf("%s: Updated zPA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zpa seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zpa wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzPAWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPACount(nCountLastUsed);
    else if (!walletdb.ReadZPACount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPAWallet::Lock()
{
    seedMaster = 0;
}

void CzPAWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPAWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPA(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPAWallet::LoadMintPoolFromDB()
{
...
void CzPAWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPAWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPAWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zpaTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPAWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPA(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zpaTracker which also adds to database
    pwalletMain->zpaTracker->Add(dMint, true);
...
void CzPAWallet::SeedToZPA(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzPAWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPAWallet::GenerateDeterministicZPA(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPAWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPAWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzPAWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
